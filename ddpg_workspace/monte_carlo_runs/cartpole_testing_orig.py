...
from cartpole import env, GAMMA, STEPS_PER_EPISODE, filename_exp, log_filename_pre
...
path_to_save = log_filename_pre+filename_exp+"/data.txt" # "results/Cartpole/exp_6/data1.txt"
f = open(path_to_save, "a")


for i in frange(0.0, 1.00001, 0.1):
    ...
    for j in range(n_samples):
        ...
        history, state_history, episode_reward, action_history = agent.test(env, nb_episodes=1, visualize=False, action_repetition=1, \
            nb_max_episode_steps=STEPS_PER_EPISODE, initial_state=[0, np.pi, 0, 0], process_noise_std=i*u_max, gamma=GAMMA)
