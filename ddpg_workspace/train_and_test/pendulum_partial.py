...
state_input = Input(shape=(1,) + (2,), name='state_input')
...
filename_exp='exp_l/lexp'
log_filename_pre = '../results/Pendulum/'
process_noise_std = 0.0 # ref to pi
theta=0.15
...
#240000
agent.fit(env, nb_steps=240000, visualize=False, callbacks=callbacks, verbose=1, nb_max_episode_steps=STEPS_PER_EPISODE, process_noise_std=process_noise_std)
...
history, state_history_nominal, episode_reward_nominal, action_history = agent.test(env, nb_episodes=1, visualize=True, action_repetition=1, \
                                                         gamma=GAMMA,nb_max_episode_steps=STEPS_PER_EPISODE , std_dev_noise=0, initial_state=[np.pi, 0.], process_noise_std=0)
